...
import { INTERFACE_ENDPOINTS, PROJECT_ENDPOINTS, OPEN_ENDPOINTS } from '../../src/constants/yapi-endpoints.js';
import {
  YapiResponse,
  ProjectInfo,
  CategoryInfo,
...
export class MockYapiClient {
   ...
   * Mock POST请求
   */
  async post<T = any>(url: string, data?: any, isFormData?: boolean): Promise<T> {
 ...
 */
export function createMockYapiClient(config?: Partial<YapiConfig>): MockYapiClient {
...
export const MockResponses = {
  ...
  interfaceInfo: {
    ...
    data: {
      ...
      req_params: [],
      req_query: [
        { name: 'page', desc: '页码', required: '0', type: 'number', example: '1' }
      ],
      req_headers: [
        { name: 'Authorization', value: 'Bearer token', desc: '认证头', required: '1' }
      ],
...
export class TypedMockResponses {
   ...
   * 创建项目信息响应
   */
  static createProjectResponse(override?: Partial<ProjectInfo>): YapiResponse<ProjectInfo> {
    return {
      errcode: 0,
      errmsg: 'success',
      data: {
   ...
   * 创建接口信息响应
   */
  static createInterfaceResponse(override?: Partial<InterfaceInfo>): YapiResponse<InterfaceInfo> {
    return {
      errcode: 0,
      errmsg: 'success',
      data: {
   ...
   * 创建错误响应
   */
  static createErrorResponse(errcode: number, errmsg: string): YapiResponse<never> {
    return {
      errcode,
      errmsg,
      data: null as never
